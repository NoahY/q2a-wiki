<?php
 ...
 define("EWIKI_UPewiki_requestLV", "rl");
 ...
 function ewiki_email_protect_edit_hook($id, &$data, &$hidden_postdata) {
   global $ewikiewikiewiki_request;

    $hidden_postdata[EWIKI_UP_NOSPAMBOT] = 1;

    if (empty($ewiki_request[EWIKI_UP_NOSPAMBOT])
        && strpos($data["content"], "@")
 ...
 function ewiki_email_protect_form($id, $data=0, $action=0, $text="PROTE1", $url="") {
    ...
    if ($url || ($email = @$ewiki_request[EWIKI_UP_ENCEMAIL])) {

          $html = "<h3>" . ewiki_t("PROTE0") . "</h3>\n";

          if (empty($ewiki_request[EWIKI_UP_NOSPAMBOT])) {  #// from GET,POST,COOKIE
 ...
 function ewiki_email_protect_feedbots() {
    ...
    if (($rl = 1 + @$ewiki_request[EWIKI_UPewiki_requestLV]) < EWIKI_FAKE_EMAIL_LOOP) {
       $html .= ",\n" . '<br><a href="' .
             ewiki_script("", EWIKI_PAGE_EMAIL,
               array(
                  EWIKI_UP_ENCEMAIL=>ewiki_email_protect_encode($m, 1),
                  EWIKI_UPewiki_requestLV=>"$rl"
               )
             ) . '">' . ewiki_t("PROTE5") . '</a><br>' . "\n";
       ($rl > 1) && sleep(3);
    }
