...
#-- init
if (!file_exists(EWIKI_CACHE_DIR)) {
...
function ewiki_get_cache($action, $id) {
   ...
   if (defined("EWIKI_CACHE_DIR") && EWIKI_CACHE_DIR) {
      $file = EWIKI_CACHE_DIR . "/" . $action . "%2F" . $id;
      if (file_exists($file)) {
         $f = gzopen($file, "r");
         if ($f) {
            ...
            if ($content) {
               $row = array(
                  "id" => $id,
                  "version" => 1,
                  "flags" => EWIKI_DB_F_BINARY¦EWIKI_DB_F_TEXT,
...
#-- if we get here, wo should store the rendered page
