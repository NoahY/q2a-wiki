...
/*
   ...
   the WikiScript interpreter (sandboxed server-side JavaScript)
*/
...
function wikiapi_getRecentChanges($since=UNIX_MILLENNIUM) {
   $r = array();
   $result = ewiki_database("GETALL", array("flags", "lastmodified"));
...
function wikiapi_getPageInfo($id, $ver=NULL) {
   $data = ewiki_database("GET", array("id"=>$id, "version"=>$ver));
   if ($data) {
...
function wikiapi_getPage($id, $ver=NULL) {
   $data = ewiki_database("GET", array("id"=>$id, "version"=>$ver));
   if ($data) {
      return(wikiapi_b64encode($data["content"]));
   }
}
