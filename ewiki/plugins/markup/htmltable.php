...
$ewiki_config["format_block"]["htmltable"] = array("&lt;table", "&lt;/table&gt;", false, 0x0003);
...
function ewiki_markup_htmltable_attrs($str) {
   if (preg_match_all('/(\s+(class|style|width|height|align|bgcolor|valign|border|cellspacing|cellpadding)=(\w+|"[^"]+"))/', $str, $uu)) {
      $str = implode("", $uu[1]);
   }
   return($str);
}
