...
@define("CALENDAR_PAGE_TITLE_REGEX","/^(.*?)".preg_quote(CALENDAR_NAME_DIVIDER) ."\d{8}$/");
@define("CALENDAR_PAGE_DATE_PARSE_REGEX", '#(.*)'. preg_quote(CALENDAR_NAME_DIVIDER) .'(\d{4})(\d{2})(\d{2})#');
...
function ewiki_calendar_list_pages(&$lines) {
    ...
    for($index=0;$index<count($lines);$index++){
                             //						1	  2				 													3
        $lines[$index]=preg_replace("#(.*>)(.*?".preg_quote(EWIKI_NAME_DIVIDER) ."\d{8})(.*)#e"," \"$1\".ewiki_calendar_page_title ('$2').'$3'",$lines[$index]);
    }
