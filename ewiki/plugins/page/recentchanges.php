...
/*
   ...
   "UpdatedPages").
- add time
- add a UseMod version
- count additional versions (of one page), GETALL only ever retrieves last
  page version
*/
...
function ewiki_page_recentchanges($recentchanges, $data, $action) {
   ...
   $result = ewiki_database("GETALL", array("meta", "lastmodified", "author"));
   $changes = array();
   $meta = array();
   while ($row = $result->get(0, 0x20)) {
      if (($row["flags"] & EWIKI_DB_F_TYPE) == EWIKI_DB_F_TEXT) {
         ...
         if ($row["lastmodified"] >= $timeframe) {
            ...
            #-- meta data array
            $meta[$id] = array();
            ewiki_page_rc_more($row, $meta[$id], $timeframe);
         }
      }
   }
...
function ewiki_page_rc_usemod(&$e, &$meta) {
   ...
   foreach ($e as $datestr => $pages) {
      ...
      foreach ($pages as $id) {
         ...
         $time = strftime($clockstr, $meta[$id][2]);
         $author = ewiki_link_regex_callback($meta[$id][0]);
   ...
   foreach ($e as $datestr => $pages) {
      ...
      foreach ($pages as $id) {
         ...
         if (count($author) >= 2) {
            ...
            #-- str_enum_unique()
            $a = array();
            foreach ($author as $i=>$str) {
               $str = $str[0];
               $a[$str][] = ($i+1);
            }
            $str = "";
            foreach ($a as $author=>$i) {
               $author = ewiki_link_regex_callback(array($author));
            ...
            $author = ewiki_link_regex_callback($author[0]);
            $log = htmlentities($meta[$id][0][1]);
         }
...
/*
   fills $x with changeLOG entries of previous page ($row) versions
*/
function ewiki_page_rc_more(&$row, &$x, $timeframe) {
   ...
   while ($ver >= 1) {
      ...
      $ver--;
      if (!$ver || !($row = ewiki_database("GET", array("id"=>$id, "version"=>$ver)))) { 
         return;
      }
   }
}
