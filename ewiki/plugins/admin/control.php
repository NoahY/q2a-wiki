...
function ewiki_action_control_page($id, &$data, $action) {
   ...
   $a_flagnames = array(
      ...
      "_APPENDONLY", "_SYSTEM", "_PART", "_UU9", "_UU10", "_UU11",
      "_UU12", "_UU13", "_UU14", "_UU15",
   );
   ...
   elseif (@$_REQUEST["pgc_setflags"]) {
      ...
      if (ewiki_database("WRITE", $data)) {
         $o .= "Page flags were updated correctly.";
         ewili_log("page flags of '$id' were set to $data[flags]");
      }
      else {
   ...
   elseif  (@$_REQUEST["pgc_rename"] && strlen($new_id = $_REQUEST["mv_to"])) {
      ...
      $exists = ewiki_database("GET", array("id"=>$new_id));
      ...
      for ($v=1; $v<=$max_ver; $v++) {
         ...
         $row = ewiki_database("GET", array("id"=>$old_id, "version"=>$v));
         $row["id"] = $new_id;
      ...
      if (empty($report)) {
         ...
         for ($v=1; $v<=$max_ver; $v++) {
            ewiki_database("DELETE", array("id"=>$old_id, "version"=>$v));
         }

         #-- adjust links/references to old page name
         if ($_REQUEST["mv_cr0"]) {

            $result = ewiki_database("SEARCH", array("refs" => $old_id));
            while ($result && ($row = $result->get())) {

               $row = ewiki_database("GET", array("id" => $row["id"]));
   ...
   elseif (@$_REQUEST["pgc_purge"] && $_REQUEST["pgc_purge1"]) {
      ...
      do {
         $verZ = $data["version"];
         while ($verZ > 0) {
            ewiki_database("DELETE", array("id"=>$id, "version"=>$verZ));
            $verZ--;
         }
      } while ($loop-- && ($data = ewiki_database("GET", array("id"=>$id))));
