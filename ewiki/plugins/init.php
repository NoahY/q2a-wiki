...
$ewiki_plugins["init"][] = "ewiki_initialization_wizard2";


function ewiki_initialization_wizard2() {
...
function ewiki_initialization_wizard($id, &$data, &$action) {
   ...
   global $ewiki_plugins, $ewiki_request;

   #-- proceed only if frontpage missing or explicetely requested
   if ((strtolower($id)=="wikisetupwizard") || ($id==EWIKI_PAGE_INDEX) && ($action=="edit") && empty($data["version"]) && !($ewiki_request["abort"])) {

      if ($ewiki_request["abort"]) {
      }

      #-- first print some what-would-we-do-stats
      elseif (empty($ewiki_request["init"])) {
